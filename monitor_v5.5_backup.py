...
def check_item(item):
    ...
    top1_qty = detect_qty(top1_title) if top1 else None
    ...
    for qty, pokota_price in sorted(pokota_prices.items()):
        ...
        if not prev_alerted and threat_exists:
            ...
            send_email("【{}】{}個セットの脅威が解消しました".format(name, qty),
    ...
    for qty, pokota_price in sorted(pokota_prices.items()):
        ...
        if not prev_alerted and threat_exists:
            ...
            send_email("【{}】{}個セットの高値上位競合が解消しました".format(name, qty),
                "【{}】{}個セットで高値上位の競合がいなくなりました\n\n■ ポコタ価格: ¥{:,}（{}位）\n\n確認時刻: {}".format(name, qty, pokota_price, pokota_ranks.get(qty, "?"), now))
    ...
    for qty, pokota_price in sorted(pokota_prices.items()):
        ...
        if not prev_alerted and threat_exists:
            ...
            send_email("【{}】{}個セットの新着1位に復帰しました！".format(name, qty),
...
def main():
    print("=" * 60)
    print("Yahoo!フリマ 価格監視ツール 起動 (v5.5 ハイブリッド)")
    print("監視①: 同額以下の競合が出たら通知")
    print("監視②: 高値なのにポコタより上位の競合が出たら通知")
    print("監視③: おすすめ1位を取られたら通知")
