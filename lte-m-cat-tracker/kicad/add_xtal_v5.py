#!/usr/bin/env python3
"""致命修正#2 v5: 40MHz水晶 Y1=HD 7D040000M01(2016/C648974)+C16/C17をU2東側に追加
...
add_seg(21.5, 25.1, 17.7, 25.1, 0.3, 'F.Cu', '+3.3V')    # スパインF（西端はR7.pad2の東）
# キャップ給電スタブ
add_seg(17.62, 25.47, 17.62, 25.175, 0.15, 'F.Cu', '+3.3V')   # C5.1
add_seg(19.22, 25.47, 19.22, 25.175, 0.15, 'F.Cu', '+3.3V')   # C6.1
# R6(13.0,21.0のまま)再給電: R6.1=+3.3V→C7.1へ / R6.2=SDA→via(14.1,22.0)系へ
add_seg(12.49, 21.3, 12.49, 21.5, 0.15, 'F.Cu', '+3.3V')
add_seg(12.49, 21.5, 12.12, 21.5, 0.15, 'F.Cu', '+3.3V')
add_seg(12.12, 21.5, 12.12, 21.75, 0.15, 'F.Cu', '+3.3V')
add_seg(13.78, 21.0, 14.1, 21.0, 0.15, 'F.Cu', 'I2C_SDA')
add_seg(14.1, 21.0, 14.1, 21.8, 0.15, 'F.Cu', 'I2C_SDA')
# R7(16.5,25.35 r180)給電: pad1東=+3.3V→C5スタブ(x17.62)へ合流
add_seg(17.25, 25.35, 17.62, 25.35, 0.15, 'F.Cu', '+3.3V')
...
add_seg(14.1, 23.3, 14.2625, 23.3, 0.15, 'F.Cu', 'I2C_SDA')  # U3.4フィーダへ

# ============ 4) SCLタップ（R7.pad2西へ東延長） ============
add_seg(15.0, 25.2, 15.75, 25.2, 0.15, 'F.Cu', 'I2C_SCL')
add_seg(15.75, 25.2, 15.75, 25.35, 0.15, 'F.Cu', 'I2C_SCL')

# ============ 5) 部品移設 ============
move_part('C5', 18.0, 25.78)
move_part('C6', 19.7, 25.78)
move_part('C8', 25.5, 25.78, 180)
move_part('R5', 27.7, 26.4, 180)
move_part('R7', 16.5, 25.35, 180)
...
add_via(20.18, 25.78, 'GND')  # C6.2
# C8.2はFジャンパで既存GND-F網(25.4,23.7系)へ（B直下にSIM_CLK/RXD縦があるためvia不可）
add_seg(25.2, 25.47, 25.2, 23.8, 0.15, 'F.Cu', 'GND')
add_seg(25.2, 23.8, 25.4, 23.8, 0.15, 'F.Cu', 'GND')
# R3.2 GNDパッド（旧スタブ削除の代替）
add_via(17.01, 24.5, 'GND')
