...
def check_item(item):
    ...
    pokota_str = ", ".join("{}個:¥{:,}({}位)".format(q, p, pokota_ranks.get(q, "?"))
                           for q, p in sorted(pokota_prices.items()))
    ...
    for i, r in enumerate(results[:SCAN_N]):
        ...
        if int(price) <= pokota_price:
            ...
            if qty not in rivals_higher:
                rivals_higher[qty] = []
            rivals_higher[qty].append({
                "rank": rival_rank,
                "price": int(price),
                "title": title[:40],
                "seller_id": seller_id,
                "price_diff": int(price) - pokota_price,
            })
    ...
    print("[{}] {} | ポコタ({}) | 安値競合: {} | 高値上位: {}".format(
        now, name, pokota_str, cheap_log, higher_log))
    ...
    for qty, pokota_price in sorted(pokota_prices.items()):
        ...
        if not prev_alerted and threat_exists:
            rival_lines = "\n".join(
                "  {}位 ¥{:,}「{}」(seller: {})".format(r["rank"], r["price"], r["title"], r["seller_id"])
                for r in qty_rivals
            )
            send_email(
            ...
            send_email(
                ...
                "■ ポコタ価格: ¥{:,}（{}位）\n\n"
                "確認時刻: {}".format(name, qty, pokota_price, pokota_ranks.get(qty, "?"), now)
            )
    ...
    for qty, pokota_price in sorted(pokota_prices.items()):
        ...
        if not prev_alerted and threat_exists:
            rival_lines = "\n".join(
                "  {}位 ¥{:,}（ポコタより+¥{}高い）「{}」(seller: {})".format(
                    r["rank"], r["price"], r["price_diff"], r["title"], r["seller_id"])
                for r in qty_rivals
            )
            send_email(
                "【{}】{}個セットで高値競合が上位に！".format(name, qty),
                "【{}】{}個セットでポコタより高い価格なのに上位に表示されている出品があります\n\n"
                "■ ポコタ価格: ¥{:,}（{}位）\n\n"
                "■ 高値上位ライバル\n{}\n\n"
                "※1円値下げ→戻しでおすすめ順リフレッシュを検討してください\n\n"
                "確認時刻: {}".format(name, qty, pokota_price, pokota_ranks.get(qty, "?"), rival_lines, now)
            )
            last_status[name]["rank_alerted"][qty] = True
        elif prev_alerted and not threat_exists:
            send_email(
                "【{}】{}個セットの高値上位競合が解消しました".format(name, qty),
                "【{}】{}個セットでポコタより高い価格で上位にいた競合がいなくなりました\n\n"
                "■ ポコタ価格: ¥{:,}（{}位）\n\n"
                "確認時刻: {}".format(name, qty, pokota_price, pokota_ranks.get(qty, "?"), now)
            )
...
def main():
    print("=" * 60)
    print("Yahoo!フリマ 価格監視ツール 起動 (v5.4)")
