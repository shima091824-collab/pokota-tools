...
def check_item(item):
    ...
    print("[{}] {} | ポコタ({}) | 脅威(上位{}件): {}".format(
        now, name, pokota_str, TOP_N, rival_log))
    ...
    for qty, pokota_price in sorted(pokota_prices.items()):
        ...
        if not prev_alerted and threat_exists:
            ...
            send_email(
                ...
                "【{}】{}個セットでポコタより安い or 同価格の出品が上位{}件に現れました\n\n"
                "■ ポコタ価格: ¥{:,}\n\n"
                "■ 脅威ライバル\n{}\n\n"
                "確認時刻: {}".format(name, qty, TOP_N, pokota_price, rival_lines, now)
            )
            ...
            send_email(
                ...
                "【{}】{}個セットでポコタより安い競合が上位{}件から消えました\n\n"
                "■ ポコタ価格: ¥{:,}\n\n"
                "確認時刻: {}".format(name, qty, TOP_N, pokota_price, now)
            )
...
def main():
    print("=" * 60)
    print("Yahoo!フリマ 価格監視ツール 起動 (v5.2)")
    print("監視条件: 上位{}件にポコタより安い or 同価格の出品が出たら通知".format(TOP_N))
    print("監視方式: セット個数別に個別比較")
    ...
    send_email(
        ...
        "圏外警告: 上位{}件から外れたら即通知\n"
        "チェック間隔: {}秒\n"
        "起動時刻: {}".format(TOP_N, CHECK_INTERVAL, datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
    )
    while True:
        for item in MONITOR_ITEMS:
            check_item(item)
